...
awful.Populate({
    ...
    thunderFocusTea = Spell(116680,  { ignoreCasting = true, ignoreMoving = true }),
    restoral = Spell(388615, { heal = true, ignoreControl = true, range = 40, ignoreFacing = true }),
    tigersLust = Spell(116841, { targeted = true }),
    invokeChiJi = Spell(325197, { ignoreMoving = true, ignoreFacing = true }),
    bloodFury = Spell(33697),
    grappleWeapon = Spell (233759, { targeted = true, cc = true, effect = "physical" }),
    provoke = Spell (115546, { targeted = true, ignoreMoving = true })
...
provoke:Callback(function(spell)
    ...
    awful.enemies.loop(function(enemy)
        ...
        if enemyCastingSpell and enemy.castTarget.isUnit(player) and provokeTable[enemyCastingSpell] and enemy.castRemains < 0.5 and enemy.distance < 5 and not spearHandStrike:Castable() then
            awful.alert({
