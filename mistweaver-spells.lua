...
tigersLust:Callback(function(spell)
    ...
    awful.enemies.loop(function(friend)
        -- Check if the friend is rooted for more than 3 seconds and enemy health is below 50%
...
invokeChiJi:Callback(function(spell)
    ...
    -- Loop through all enemy units
    awful.fgroup.loop(function(friend)
        -- Check if the friendly is rooted for more than 3 seconds and enemy health is below 50%
        if friend.rootRemains > 3 and enemy.hp < 50 then
            return invokeChiJi:Cast()
        end
    end)
