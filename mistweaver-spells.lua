...
function rotation()
    ...
    if tigerPalm:Castable(target) and player.lastCast ~= "tigerPalm" then
        tigerPalm:Cast(target)
        player.lastCast = "tigerPalm"
        return
    end

    -- If Tiger Palm has just been cast, then cast Blackout Kick.
    -- This ensures that Tiger Palm is always cast before Blackout Kick.
    if blackoutKick:Castable(target) and player.lastCast == "tigerPalm" then
        ...
        player.player.lastCast = "blackoutKick"
        return
    end
end
