...
function rotation()
    ...
    if GetTime() - lastRisingSunKick >= 10 then
        if risingSunKick:CanCast(target) then
    ...
    -- If Rising Sun Kick is on cooldown, we move to the next part of the rotation: Tiger Palm and Blackout Kick
    -- Cast Tiger Palm if it's off cooldown.
    if tigerPalm:CanCast(target) then
        tigerPalm:Cast(target)
        lastTigerPalm = GetTime()
        return
    end

    -- If Tiger Palm has been cast in the last GCD, then cast Blackout Kick.
    -- This ensures that Tiger Palm is always cast before Blackout Kick.
    if GetTime() - lastTigerPalm <= 1.5 and blackoutKick:CanCast(target) then
