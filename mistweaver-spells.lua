...
invokeChiJi:Callback(function(spell)
    ...
    if (player.rootRemains > 3 or player.slowed) and player.hp < 75 then
        awful.alert({
    ...
    awful.friends.loop(function(friend)
        -- Check if the friend is rooted for more than 3 seconds and their health is below 50%
        if (friend.rootRemains > 3 or player.slowed) and friend.hp < 80 or friend.hp < 80 and target.distance > 5 then
            awful.alert({
                message="Casted Chi-Ji, the Red Crane!", 
                texture=325197,
                })
    ...
    awful.friends.loop(function(friend)
        -- Check if the enemy is rooted for more than 3 seconds and their health is below 50%
        if (friend.rootRemains > 3 or player.slowed) and friend.target.hp < 80 or friend.hp < 80 and target.distance > 5 then
            awful.alert({
                message="Casted Chi-Ji, the Red Crane!", 
                texture=325197,
                })
...
paralyze:Callback(function(spell)
    ...
    if enemyHealer.distance <= paralyze.range and target.hp < 70 and paralyze:Castable(enemyHealer) and enemyHealer.ccRemains < awful.buffer + awful.latency and enemyHealer.incapDR >= 0.25 and not (player.target.guid == enemyHealer.guid) then
